...
var (
	startWorkChan = make(chan struct{}, 10)
	OutDir        = "./out"
)
...
func (s *SchoolMaker) CreateSchool(schoolName string) types.School {
	...
	go func() {
		...
		for i := 1; i <= TotalClasses; i++ {
			...
			// pass empty struct to channel if channel full then wait here until other complete
			startWorkChan <- struct{}{}
	...
	for i := 1; i <= TotalClasses; i++ {
		...
		// fee the channel on each receive
		<-startWorkChan
	}
